  ...
  scheduleJob('* * * * *', async () => {
    ...
    for (const user of elegibleUsers) {
      ...
      if (currentDay === day && currentMonth === month) {
        await sendBirthdayMessage(user.guildId, user.userId)
      }
    }
  })
}
...
const sendBirthdayMessage = async (guildId: string, userId: string) => {
  // Get main channel from server preferences
  const mainChannel = settingsHandler.fetchSetting(guildId, Settings.mainchannel)
  if (mainChannel === null) {
    container.client.channels.cache.get(mainChannel)
  }
}
